...
private:
  ...
  void cloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    // floor detection 获得地面的法向量
    boost::optional<Eigen::Vector4f> floor = detect(cloud);//获得平面的法向量

    // publish the detected floor coefficients //自定义的消息类型
    hdl_graph_slam::FloorCoeffs coeffs;
    coeffs.header = cloud_msg->header;
    if(floor) {
      coeffs.coeffs.resize(4);//就是传一个数组
      for(int i=0; i<4; i++) {
  ...
  boost::optional<Eigen::Vector4f> detect(const pcl::PointCloud<PointT>::Ptr& cloud) const {
    ...
    tilt_matrix.topLeftCorner(3, 3) = Eigen::AngleAxisf(tilt_deg * M_PI / 180.0f, Eigen::Vector3f::UnitY()).toRotationMatrix();//传感器安装的倾斜补偿
  ...
  //发布地面电云
    ...
    ransac.getModelCoefficients(coeffs);//获得平面的法向量

    double dot = coeffs.head<3>().dot(reference.head<3>());//拟合的法向量和传感器的Z轴方向计算内积 垂直的话应该是
    if(std::abs(dot) < std::cos(floor_normal_thresh * M_PI / 180.0)) { // 阈值是cos(10°)
      // the normal is not vertical
      return boost::none;
    }

    // make the normal upward 确保坐标轴朝上
    if(coeffs.head<3>().dot(Eigen::Vector3f::UnitZ()) < 0.0f) { //和z轴相乘，如果小于0，说明方向反了
      coeffs *= -1.0f;
    }
    ...
    if(floor_points_pub.getNumSubscribers()) {
      ...
      extract.setInputCloud(filtered);//初步过滤之后的点云
      extract.setIndices(inliers);//ransac拟合得到的平面点的index
      extract.filter(*inlier_cloud);//通过extractIndices来提取点云。
