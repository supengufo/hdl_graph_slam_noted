...
private:
  ...
  void cloud_callback(pcl::PointCloud<PointT>::ConstPtr src_cloud) {
    ...
    if(!base_link_frame.empty()) {
      if(!tf_listener.canTransform(base_link_frame, src_cloud->header.frame_id, ros::Time(0))) {//查找base_link - >velodyne的转换
        std::cerr << "failed to find transform between " << base_link_frame << " and " << src_cloud->header.frame_id << std::endl;
      }

      tf::StampedTransform transform;// base_link -> velodyne
      ...
      pcl_ros::transformPointCloud(*src_cloud, *transformed, transform);//转换到base_link坐标系下
