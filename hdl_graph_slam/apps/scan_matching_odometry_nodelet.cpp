...
private:
  ...
  Eigen::Matrix4f matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    if(!keyframe) {//没有keyframe的话 初始化一下
    ...
    if(!registration->hasConverged()) {
      ...
      return keyframe_pose * prev_trans;//没有收敛 则返回上一个keyframe的位姿
    }
    //odom是一直更新的
    Eigen::Matrix4f trans = registration->getFinalTransformation();//diff transformation
    Eigen::Matrix4f odom = keyframe_pose * trans;//update transformation  T_wi*T_i_i+1
    //prev_trans = T_i-1_i
    //如果间隔帧太远 则认为是错误的 扔掉
    if(transform_thresholding) {//这个地方写的不对吧？ TODO 是不是没有inverse 如果是单位阵的话 就好理解了
      Eigen::Matrix4f delta = prev_trans.inverse() * trans;// T_i_i-1*T_i_i+1
    ...
    //odom -> velodyne ???
    //odom -> keyframe ???
    auto keyframe_trans = matrix2transform(stamp, keyframe_pose, odom_frame_id, "keyframe");//为什么要发布前一帧的位姿？对的。因为到后面还要判断delta大于一定的值才会发布出去，不然暂时就发布上一帧的
  ...
  void publish_odometry(const ros::Time& stamp, const std::string& base_frame_id, const Eigen::Matrix4f& pose) {
    ...
    odom.header.frame_id = odom_frame_id;//odom
