...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    if(base_frame_id.empty()) {//velodyne
      base_frame_id = cloud_msg->header.frame_id;
    }

    if(!keyframe_updater->update(odom)) {//第一帧的时候 不更新odom
    ...
    std::lock_guard<std::mutex> lock(keyframe_queue_mutex);//TODO 这里为什么要加锁？ 是因为有地方要用keyframe_queue？
   ...
   */
  //1.对callback函数里keyframe_queue保存的前10帧添加到new_keyframes里并删掉原来的
  //2.把这几帧加到图优化里面
  bool flush_keyframe_queue() {
    ...
    //对于缓冲区的每一帧，添加到图优化里面
    int num_processed = 0;
    for(int i=0; i<std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) {//max_keyframes_per_update最多从callback的keyframe_queue里面转移10帧到new_keyframes
      num_processed = i;

      const auto& keyframe = keyframe_queue[i];
      // new_keyframes will be tested later for loop closure
      new_keyframes.push_back(keyframe);//把callback函数里的消息添加到keyframe_queue里面，然后在

      // add pose node
      Eigen::Isometry3d odom = odom2map * keyframe->odom;
      keyframe->node = graph_slam->add_se3_node(odom);//pose就是顶点
      keyframe_hash[keyframe->stamp] = keyframe;//利用hash表来保存keyframe

      // fix the first node 初始化
      if(keyframes.empty() && new_keyframes.size() == 1) {
        if(private_nh.param<bool>("fix_first_node", false)) {
          Eigen::MatrixXd inf = Eigen::MatrixXd::Identity(6, 6);//信息矩阵 固定第一帧？TODO 这个地方如何理解？
          std::stringstream sst(private_nh.param<std::string>("fix_first_node_stddev", "1 1 1 1 1 1"));
          for(int i = 0; i < 6; i++) {
            double stddev = 1.0;
            sst >> stddev;
            inf(i, i) = 1.0 / stddev;//这个地方的信息矩阵是协方差的倒数？所以是1.
          }
      ...
      // add edge between consecutive keyframes 这里每次清空的时候保留了上一帧？
      const auto& prev_keyframe = i == 0 ? keyframes.back() : keyframe_queue[i - 1];
      //后添加边
      Eigen::Isometry3d relative_pose = keyframe->odom.inverse() * prev_keyframe->odom;// T_i_{i+1}
      Eigen::MatrixXd information = inf_calclator->calc_information_matrix(keyframe->cloud, prev_keyframe->cloud, relative_pose);//计算相邻两帧的信息矩阵
    ...
    keyframe_queue.erase(keyframe_queue.begin(), keyframe_queue.begin() + num_processed + 1);//清空callback函数的keyframe_queue的前面这些帧
    return true;
  }
  ...
  void floor_coeffs_callback(const hdl_graph_slam::FloorCoeffsConstPtr& floor_coeffs_msg) {
    ...
    floor_coeffs_queue.push_back(floor_coeffs_msg);//每一帧
  }
  ...
  bool flush_floor_queue() {
    ...
    const auto& latest_keyframe_stamp = keyframes.back()->stamp;//图优化结束之后的最后一帧

    bool updated = false;
    for(const auto& floor_coeffs : floor_coeffs_queue) { //对于地面法向量队列里的每一帧数据
      if(floor_coeffs->header.stamp > latest_keyframe_stamp) {//找到早于图优化结束之后的最后一帧前的所有的地面帧
        break;
      }
      //因为keyframe_hash存储的是已经添加到图优化里面的关键帧，所以在里面寻找对应的帧的地面法向量。
      auto found = keyframe_hash.find(floor_coeffs->header.stamp);//找到和地面法向量对应的关键帧
      if(found == keyframe_hash.end()) {//没找到 退出
        continue;
      }
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    // loop detection
    // keyframes:所有的历史关键帧
    // new_keyframes:上一次优化循环中新添加的关键帧
    // 意思就是说，在历史所有的关键帧里面，搜索新添加的关键帧是否有能够匹配到一起的。
    // 返回所有的回环
    std::vector<Loop::Ptr> loops = loop_detector->detect(keyframes, new_keyframes, *graph_slam);
    // 计算所有的回环两帧之间的信息矩阵
    // 添加到图优化边（这里只需要添加边就可以了）
    ...
    //然后把滑窗里的关键帧，添加到历史帧里。并对滑窗清空。
    std::copy(new_keyframes.begin(), new_keyframes.end(), std::back_inserter(keyframes));//keyframes_vec更新处。在
    new_keyframes.clear();//???
    ...
    std::transform(keyframes.begin(), keyframes.end(), snapshot.begin(),//保存到snapshot里面
